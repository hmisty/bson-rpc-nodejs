...
Proxy.prototype.connected = false; //if connected
...
Proxy.prototype.connect = function(on_connected_callback) {
	...
	this.connection.on('error', () => {
		...
		if (this.reconnect_try < this.MAX_RECONNECT_TRY ||
			...
			setTimeout(() => {
				/* Due to connect delay, a former re-connect try may succeed
				 * and change this.connected to true when this re-connect try timeout.
				 * If so, we do not need to connect again.
				 */
				if (this.connected == false) {
					console.log('try to re-connect:', ntry);
					this.connection.connect(this.port, this.host, () => {
						/* Be careful that we will have many callbacks being called 
						 * once successfully connected or re-connected,
						 * only if that callback has not yet been called.
						 * So we use this.connected to secure only one of them
						 * will really work and others just passed by.
						 */
						if (this.connected == false) {
							console.log('re-connected');
							this.connected = true;
							this.reconnect_try = 0;
						}
					});
				}
	...
	this.connection.connect(this.port, this.host, () => {
		if (this.connected == false) {
			...
			this.connected = true;
			this.reconnect_try = 0; //reset it once successfully connected
