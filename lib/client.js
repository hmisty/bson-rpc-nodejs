...
Proxy.prototype.MAX_RECONNECT_TRY = 5; 
...
Proxy.prototype.connect = function(on_connected_callback) {
	...
	var connect = () => {
		this.connection.connect(this.port, this.host, on_connected_callback);
	};
	
	this.connection.on('data', data_handler);
	this.connection.on('error', () => {
		if (this.reconnect_try++ < this.MAX_RECONNECT_TRY) {
			console.log('try to re-connect:', this.reconnect_try);
			//setTimeout(connect, this.reconnect_try * 1000); //more delay for next retry
			connect();
		}
	});
